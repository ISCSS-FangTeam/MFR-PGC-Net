...
class Compose():
    ...
    def __call__(self, img, bboxes, bg_mask=None, edge_mask=None): 
        for tr in self.transforms:
            img, bboxes, bg_mask, edge_mask = tr(img, bboxes, bg_mask, edge_mask) 
        return img, bboxes, bg_mask, edge_mask
...
class RandomScale():
    ...
    def __call__(self, img, bboxes, bg_mask=None, edge_mask=None):
        ...
        if edge_mask is not None:
            edge_mask = cv2.resize(edge_mask, None, fx=scale, fy=scale, interpolation=cv2.INTER_NEAREST)
        return img, bboxes, bg_mask, edge_mask
...
class ResizeRandomCrop():
    ...
    def __call__(self, img, bboxes, bg_mask=None, edge_mask=None):
        ...
        if edge_mask is not None:
            edge_mask = cv2.copyMakeBorder(edge_mask, padH,padH,padW,padW, cv2.BORDER_CONSTANT, value=0)
            edge_mask = edge_mask[new_hmin:new_hmin+crop_H, new_wmin:new_wmin+crop_W]
        return img, bboxes, bg_mask, edge_mask
...
class Normalize_Caffe():
    ...
    def __call__(self, img_RGB, bboxes, bg_mask=None, edge_mask=None):
